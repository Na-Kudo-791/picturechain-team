...
window.addEventListener('DOMContentLoaded', () => {
  const chars = "あいうえおかきくけこさしすせそたちつてとなにぬねのはひふへほまみむめもやゆよ";
  const firstChar = chars[Math.floor(Math.random() * chars.length)];

  const firstCharText = document.getElementById("firstCharText");
  if (firstCharText) {
    firstCharText.textContent = `最初の文字は「${firstChar}」`;
  }

  // 必要なら、他ページに渡すため保存もできる
...
/*
  window.addEventListener('DOMContentLoaded', () => {
    ...
    if (lastChar) {
        nextCharText.textContent = `次の文字は「${lastChar}」`;
    }
});
*/